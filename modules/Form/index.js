...
const Form = () => {
  const urlHobbies = 'http://localhost:3000/hobbies'
  const urlMovies = 'http://localhost:8000/movies'
  const { data: listHobbies } = useFetch(urlHobbies)
  const { data: top100Films } = useFetch(urlMovies)
            ...
            onChange={(event, value) => {      
              ...
              value.map(hobbies => {
                if (hobbies.id == 1) {
                  ...
                  value.splice(value.indexOf(hobbies.id), 1)
                  setOpen(true)
                  console.log(value)
                }
              })
